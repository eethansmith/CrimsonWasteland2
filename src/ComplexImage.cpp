...
ComplexImage::ComplexImage(std::shared_ptr<RawImageData> theData)
	: theData(theData)
	, m_pRescaleSurface(nullptr)
	, m_iTransparencyColour(-1)
{
...
bool ComplexImage::exists() const { return theData != nullptr; }
...
bool ComplexImage::adjustXYWidthHeight(DrawingSurface* pTarget,
...
void ComplexImage::renderImage(DrawingSurface* pTarget,
	int iXSource, int iYSource,
...
*/
void ComplexImage::renderImageWithMask(DrawingSurface* pTarget,
	int iXSource, int iYSource,
	int iXTarget, int iYTarget,
...
void ComplexImage::renderImageWithMaskAndTransparency(DrawingSurface* pTarget,
	int iXSource, int iYSource,
	int iXTarget, int iYTarget,
	int iWidth, int iHeight,
...
*/
void ComplexImage::renderImageBlit(BaseEngine* pEngine, DrawingSurface* pTarget,
...
*/
void ComplexImage::renderImageApplyingMapping(BaseEngine* pEngine, DrawingSurface* pTarget,
	...
	ImagePixelMapping* mapping) const
{
	...
	for (int iYOffset = yMinOffset; iYOffset < yMaxOffset; ++iYOffset)
	{
		for (int iXOffset = xMinOffset; iXOffset < xMaxOffset; ++iXOffset)
		{
			dx = iXOffset;
			dy = iYOffset;
			if (mapping->mapCoordinates(dx, dy, *m_pSimpleImage))
			{
				int iColour = 0xFFFFFF & theData->getRawPixelColour((int)(dx + 0.5), (int)(dy + 0.5));
				if (iColour != 0xFFFFFF) // Check if color is not white
				{
